...
function ENT:CustomOnInitialize()
	...
	if self:IsValid() == true then 
		if self.Dead == false then
			timer.Simple(self.Skill_Time,function() if IsValid(self) then self:GetSkill() end 
				end)
		end
	end
...
function ENT:GetSkill()
	if self:IsValid() == true then 
		if self.Dead == false then
			self:VJ_ACT_PLAYACTIVITY("zbs_skill_walk",true,3.5,false)

			self.RangeAttacking = true

			VJ_EmitSound(self,self.SoundTbl_Setup,100,100)

			timer.Simple(0.5,function() if IsValid(self) then self:DropNeedle() end 
				end)

			timer.Simple(65.0,function() if IsValid(self) then self:GetSkill() end 
				end)
		end
	end
end
