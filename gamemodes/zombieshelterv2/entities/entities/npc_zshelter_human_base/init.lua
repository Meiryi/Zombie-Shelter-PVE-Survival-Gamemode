...
ENT.Model = {"models/zshelter/bots/bot_rocket.mdl"}
ENT.StartHealth = 100
...
ENT.RangedAttackDistance = 1024
ENT.DeathAnimations = {
	"head", "gutshot", "back", "crouch_die", "left",
}
...
function ENT:DoKilled(dmginfo)
	...
	if(self.HasDeathAnimation) then
		...
		local seq = VJ_PICK(self.DeathAnimations)
		timer.Simple(0, function()
			if(!IsValid(self) || !seq) then return end
			self:ResetSequence(seq)
			self:VJ_ACT_PLAYACTIVITY(seq, true, 1, true, 0)
		end)
