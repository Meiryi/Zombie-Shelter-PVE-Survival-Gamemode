...
SWEP.Category				= "#zshelter.category.equipment" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep.
SWEP.Author				= "Meika, ★Bullet★, Kamikaze" --Author Tooltip
SWEP.Contact				= "" --Contact Info Tooltip
SWEP.Purpose				= "#zshelter.weapon_help.red_dragon_cannon" --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "#zshelter.weapon.red_dragon_cannon"		-- Weapon name (Shown on HUD)
...
sound.Add({
	['name'] = "CannonEX.Fire",
	['channel'] = CHAN_STATIC,
	['sound'] = { "weapons/tfa_cso/cannonex/cannonex_shoota.wav"},
	['pitch'] = {100,100}
})

TFA.AddWeaponSound ( "Cannonex.Transform", "weapons/tfa_cso/cannonex/cannonex_dtransform.wav" )

--Firing related
SWEP.Primary.Sound 			= Sound("CannonEX.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage		= 6000					-- Damage, in standard damage points.
...
SWEP.ProjectileDamage = 6000

--[[VIEWMODEL]]--

SWEP.ViewModel			= "models/weapons/tfa_cso/c_cannonex.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_cannonex.mdl" -- Weapon world model path
...
SWEP.BaseBlastDamage = (engine.ActiveGamemode() == "sandbox" and 6000) or 12500 -- Meiryi : bruh what the fuck
SWEP.SkillDamage = (engine.ActiveGamemode() == "sandbox" and 800) or 2400
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_cannonex")
...
function SWEP:SecondaryAttack()
	...
	local t = self:GetActivityLength(tanim)
	self:SetStatusEnd(CurTime() + (t * 0.925))
	self.FailTimer = CurTime() + t * 0.975
	self.UnTransTime = CurTime() + 12
	timer.Simple(t * 0.925, function()
		local effectdata = EffectData()
		effectdata:SetOrigin(owner:GetPos() + Vector(0, 0, 5))
		effectdata:SetAngles(Angle(0,0,0))
		util.Effect("exp_gunkata", effectdata)
		owner:EmitSound( "weapons/tfa_cso/cannonex/cannonexplo.wav" )
		self.AllowedtoDamage = false
		if SERVER then
			local dmginfo = CSO:DMGInfo(self.Owner, self, self.SkillDamage, Vector(0,0,0), DMG_BLAST, self:GetPos())
			CSO:DoRadiusAttack(self, dmginfo, 300, false, 0, 1, false, nil, true)
		end
	end)
end

function SWEP:PostPrimaryAttack()
	if(self:GetSilenced() && self.UnTransTime < CurTime()) then
		CSO:ForceVMSequenceEvent(self, 7, -1, false, true, function()
			self:ChooseSilenceAnim(!self:GetSilenced())
			self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
			local t = self:GetActivityLength(tanim)
			self:SetStatusEnd(CurTime() + t)
			self:EmitSound( "weapons/tfa_cso/cannonex/cannonex_d_reload2.wav" )
		end)
	else
		if(self:GetSilenced() && self.FailTimer > CurTime()) then
			if(CLIENT) then return end
			self.Owner.DragonAliveTimer = CurTime() + 8
			local dragon_ = ents.Create("cannonex_dragon")
				dragon_:SetOwner(self.Owner)
				dragon_:Spawn()
		end
	end
end