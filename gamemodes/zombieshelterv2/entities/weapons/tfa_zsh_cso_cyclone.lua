...
SWEP.PrintName				= "#zshelter.weapon.cyclone"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.RPM_Burst				= 1000					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
...
SWEP.TracerName 		= "zsh_cso_tra_cyclone" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Think2( ... )
    ...
    if(shooting) then
    	if(!self.Shooting) then
    		self:EmitSound("weapons/tfa_cso/cyclone/fire_start.wav")
    		self.StartEmitSound = CurTime() + 0.1
    	end
    	if(self.StartEmitSound < CurTime() && self.NextLoop < CurTime()) then
    		self:EmitSound("shigure/cyclone_loop.wav")
    		self.NextLoop = CurTime() + 3.5
    	end
    	self.LastFireTime = CurTime()
    else
    	if(!shooting && self.Shooting) then
    		if(self:GetStatus() != TFA.GetStatus("reloading")) then
				self:SendViewModelAnim(ACT_VM_PRIMARYATTACK_EMPTY)
			end

    		self:StopSound("shigure/cyclone_loop.wav")
    		self.NextLoop = 0
    	end
    end
...
local mat = Material("tracers/tracer_cyclone")
SWEP.Alpha = 0
function SWEP:PostDrawViewModel(vm, wep, ply)
	if(CurTime() - self.LastFireTime <= 0.05) then
		self.Alpha = 255
	else
		self.Alpha = math.Clamp(self.Alpha - RealFrameTime() * 2400, 0, 255)
	end
	...
	local endpos = util.TraceLine({
		start = pos,
		endpos = pos + ply:EyeAngles():Forward() * 32767,
		filter = ply,
		collisiongroup = COLLISION_GROUP_DEBRIS,
	}).HitPos
	render.SetMaterial(mat)
	render.DrawBeam(pos, endpos, 10, 0, 1, Color(255, 255, 255, self.Alpha))
end