...
SWEP.Category				= "#zshelter.category.rifle" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep.
...
SWEP.PrintName				= "#zshelter.weapon.ak47_paladin"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.Spread		= .0042					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy = .0021	-- Ironsight accuracy, should be the same for shotguns
...
SWEP.JumpAccuracyMultiplier= 5--Less is more.  Accuracy * 2 = Half as accurate.  Accuracy * 5 = 1/5 as accurate
SWEP.WalkAccuracyMultiplier= 3--Less is more.  Accuracy * 2 = Half as accurate.  Accuracy * 5 = 1/5 as accurate
...
function SWEP:Think2(...)
	...
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
		...
		self.Primary_TFA.KickUp = 0.66
		self.Primary_TFA.KickDown = 0.48
		self.Primary_TFA.KickHorizontal = 0.405
		self.Primary_TFA.AmmoConsumption = 1
		self.Primary_TFA.NumShots = 1
		self.MuzzleFlashEffect = "cso_muz_pal"
		self.VolumeMultiplier = 8
