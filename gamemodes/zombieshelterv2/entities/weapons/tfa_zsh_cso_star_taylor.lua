...
SWEP.TracerName 		= "zsh_cso_tra_star_taylor" 	--Change to a string of your tracer name.  Can be custom.
...
function SWEP:Think2( ... )
    ...
    if(shooting) then
    	if(self.NextLoop < CurTime()) then
    		self:EmitSound("weapons/tfa_cso/star_taylor/fire.wav")
    		self.NextLoop = CurTime() + 4
    	end
    	...
    	if(!shooting && self.Shooting) then
    		...
    		self:StopSound("weapons/tfa_cso/star_taylor/fire.wav")
    		self.NextLoop = 0
    	end
    end
...
local mat = Material("tracers/ef_y22s2sfpistol_laser")
SWEP.Alpha = 0
function SWEP:PostDrawViewModel(vm, wep, ply)
	...
	if(self.Alpha <= 0 || self:GetStatus() == TFA.GetStatus("reloading")) then
		return
	end
	...
	render.DrawBeam(pos, endpos, 4, 1 - r, r, Color(255, 255, 255, self.Alpha))
end