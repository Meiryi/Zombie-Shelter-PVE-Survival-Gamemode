...
SWEP.Purpose				= "#zshelter.weapon_help.thanatos7" --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "#zshelter.weapon.thanatos7"		-- Weapon name (Shown on HUD)
...
SWEP.Secondary.Ent = "zsh_thanatos7_blade"
...
function SWEP:TestSecondary()
	if SERVER then
		...
		ent.BaseAngle = self.Owner:EyeAngles()
		ent:Spawn()
		ent:Activate()
	end
	...
	if SERVER then
		if self:Ammo2() <= 0 then 
			if(self.Nextregen <= CurTime()) then
				self.Owner:GiveAmmo( 1,"Thanatos7_Blade", true )
				self.Nextregen =  CurTime()  + 15
			end
		else
			self.Nextregen =  CurTime()  + 15
		end
