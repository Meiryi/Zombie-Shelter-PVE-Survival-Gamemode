...
SWEP.PrintName				= "#zshelter.weapon.vulcanus7"		-- Weapon name (Shown on HUD)
...
SWEP.BModeTriggerTime = CurTime() + 15
SWEP.TriggerTime = 10

function SWEP:SecondaryAttack()
	if (not self:CanPrimaryAttack() || !self:GetSilenced()) then return end
	...
	self.BModeTriggerTime = CurTime() + self.TriggerTime
...
function SWEP:Think2(...)
	if(!self:GetSilenced()) then
		if(!CSO:BlockExtraActionTime(self)) then
			if(self.BModeTriggerTime < CurTime() && self.CanToggle) then
				self:ChooseSilenceAnim( not self:GetSilenced() )
				self:SetStatus(TFA.Enum.STATUS_SILENCER_TOGGLE)
				self:SetStatusEnd(CurTime() + self:GetActivityLength( tanim ))
				if(SERVER) then
					self.Owner:GiveAmmo(1, "missile_vulcanus7")
				end
...
function SWEP:PostPrimaryAttack()
	...
	if(self:Clip1() % 5 == 0) then
		...
		if(CLIENT) then
			...
			local e = EffectData()
			e:SetOrigin(util.TraceLine({
				start = self.Owner:EyePos(),
				endpos = self.Owner:EyePos() + (self.Owner:EyeAngles() + self.Owner:GetViewPunchAngles()):Forward() * 32767,
				filter = self.Owner,
				mask = MASK_SHOT,
				collisiongroup = COLLISION_GROUP_DEBRIS,
			}).HitPos)
			e:SetStart(vm:GetAttachment(1).Pos)
			e:SetAttachment(1)
			util.Effect("tra_zsh_valcanus7", e)
		end
		self:EmitSound("Vulcanus7.Fire2")
		self:ShootBullet(150, 2, 2, 0, true, true)
		self.TracerName = nil
	else
		if(self:Clip1() % 5 == 1 && SERVER) then
