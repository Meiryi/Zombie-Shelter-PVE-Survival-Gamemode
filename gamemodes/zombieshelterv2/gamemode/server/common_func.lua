...
function ZShelterVisible_Vec(self, vec, target)
	...
	local ret = util.TraceLine(tr)
	local ent = ret.Entity
	return ret.Fraction == 1 || (!ret.HitWorld && IsValid(ent) && !ZShelter_IsProp(ent:GetClass()) && !ent.IsBuilding && target.LastStuckTime && target.LastStuckTime > CurTime()) || (IsValid(ent) && ZShelter_WhiteListedEntity[ent:GetClass()]) -- This is terrible
end
