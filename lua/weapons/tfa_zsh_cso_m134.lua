...
SWEP.Primary.RPM_Semi				= 1500					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
...
--Recoil Related
SWEP.Primary.KickUp			= 1.35					-- This is the maximum upwards recoil (rise)
SWEP.Primary.KickDown			= 1.35					-- This is the maximum downwards recoil (skeet)
...
SWEP.Primary.SpreadIncrement = 0.1 --What percentage of the modifier is added on, per shot.
...
SWEP.TargetSpinupTime = 0.85
SWEP.CachedStatus = false
SWEP.rate = 1
SWEP.SpinupSound = Sound("weapons/tfa_cso/m134/spinup.wav")
SWEP.SpindownSound = Sound("weapons/tfa_cso/m134/spindown.wav")
SWEP.ShoudPlaySound = false
...
function SWEP:Think2( ... )
    ...
    local shooting = self:GetStatus() == TFA.GetStatus("shooting")
    local reloading = self:GetStatus() == TFA.GetStatus("reloading")
    local reloading_wait = self:GetStatus() == TFA.GetStatus("reloading_wait")
    local draw = self:GetStatus() == 1
    if(draw || reloading || reloading_wait) then
    	self:SetNextPrimaryFire(CurTime() + 0.15)
    end
    if(owner:IsPlayer() && !CSO:BlockExtraAction(self) && self:Clip1() > 0) then
    	if(!owner:KeyDown(1)) then
    		if(self.__KeyDown) then
	    		self:SetPlaybackRate(1)
	    		self:SendViewModelAnim(ACT_VM_PRIMARYATTACK_EMPTY)
	    		self:EmitSound(self.SpindownSound)
	    		self:SetNextIdleAnim(CurTime() + self.SpinupTime)
    		end
			self.__KeyDown = false
		else
			if(!self.__KeyDown) then -- setup spin timer
				self.rate = 0
				self.ShoudPlaySound = false
				self.SpinupTime = CurTime() + self.TargetSpinupTime
				self:SetStatusEnd(self.SpinupTime)
				CSO:ForceVMSequenceNoStatus(self, 6)
			end
			self.__KeyDown = true
    	end
    else
    ...
    if(!shooting) then -- check diff
    	if(self.CachedStatus == true) then
    		self:SendViewModelAnim(ACT_VM_PRIMARYATTACK_EMPTY)
    		self:EmitSound(self.SpindownSound)
    	end
    end
    self.CachedStatus = shooting
    BaseClass.Think2(self,...)
end

function SWEP:PrimaryAttack()
	if(self:GetStatus() == TFA.GetStatus("reloading") || self:GetStatus() == TFA.GetStatus("reloading_wait") || self:GetSprinting()) then return end
	if(self.SpinupTime > CurTime()) then
		if(!self.ShoudPlaySound) then
			self:SetNextIdleAnim(CurTime() + self.SpinupTime)
			self:EmitSound(self.SpinupSound)
			self.ShoudPlaySound = true
		end
		self.rate = (1 - math.max(self.SpinupTime - CurTime(), 0) / self.TargetSpinupTime)
		local vm = self:GetOwner():GetViewModel()
		vm:SetPlaybackRate(self.rate)
		return
	else
...
function SWEP:Reload(...)
	if(self.__KeyDown) then return end
